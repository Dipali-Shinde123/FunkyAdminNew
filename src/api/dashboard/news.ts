...
export const useUpdateNews = ()=>{
    ...
    const updatenews = async(newsId: string | number, formData: FormData)=>{
        if(!newsId)return {success: false,message:'Music Id is required.'};
        const URL =`${HOST_API}${endpoints.news.update}/${newsId}`;
        ...
        try{
            ...
            console.error('Error Response:', axiosError.request?.data);
            const message = axiosError?.response?.data?.message || 'Error updating the news.';
